...
import os
import re
import psycopg2 # <--- Используем драйвер PostgreSQL
import datetime
...
# Мы берем токен и URL базы из "Environment Variables" (это мы настроим на сервере)
...
# Жылды автоматты түрде анықтаймыз
...
# --- 3. PostgreSQL БАЗА ДАННЫХ (Реальный Код) ---

# init_db() больше не нужна, т.к. мы создали таблицы вручную в Этапе 2

def check_user_registration(user_id: int) -> bool:
    """Проверяет, есть ли user_id в таблице users."""
    try:
        # 'with' сам открывает и закрывает соединение
        ...
        logging.error(f"DB check_user_registration error: {error}")
        return False
...
def register_user_in_db(user_id: int, fio: str) -> bool:
    ...
    try:
        with psycopg2.connect(DATABASE_URL) as con:
            with con.cursor() as cur:
                cur.execute("INSERT INTO users (user_id, fio) VALUES (%s, %s)", (user_id, fio))
                con.commit() # Сохраняем изменения
        return True
    except (Exception, psycopg2.Error) as error:
        logging.error(f"DB register_user_in_db error: {error}")
        return False
...
def save_grade_to_db(user_id: int, subject: str, grade: int) -> bool:
    ...
    try:
        with psycopg2.connect(DATABASE_URL) as con:
            with con.cursor() as cur:
                # Дата будет добавлена автоматически (DEFAULT CURRENT_DATE в SQL)
                cur.execute(
        ...
        logging.error(f"DB save_grade_to_db error: {error}")
        return False
    ...
    try:
        with psycopg2.connect(DATABASE_URL) as con:
            with con.cursor() as cur:
                ...
                query = "SELECT subject, grade FROM grades WHERE user_id = %s"
                params = [user_id]
                
                # month параметрі '2025-10' немесе 'All' форматында келеді
                if month != 'All':
                    # Используем 'TO_CHAR' для извлечения 'YYYY-MM' из даты
        ...
        logging.error(f"DB fetch_grades_from_db error: {error}")
...
def main() -> None:
    ...
    # Проверяем, есть ли ссылка на базу
    if not DATABASE_URL:
